...
let getFood = (dishName) => {
  document.querySelector(".container .displaying-message").innerHTML = "Please wait...";
  ...
  fetch(url + dishName)
    ...
    .then((data) => {
      ...
      if (myMeal) {
        ...
        resultBox.innerHTML = `<p>No results found for "${dishName}"</p>`;
      }
    })
    .catch((error) => {
      console.error(error);
      document.querySelector(".container .displaying-message").innerHTML = "An error occurred. Please try again.";
    });
};
